class Accordion {
    constructor() {
        this.accordionContainer = document.querySelector('.accordion-coord-changer');
        this.initAccordion();
    ...
    initAccordion() {
        document.querySelectorAll('.accordion-coord-changer__label').forEach(label => {
            label.addEventListener('click', () => {
                const contentBox = label.parentElement;

                document.querySelectorAll('.accordion-coord-changer__contentBx').forEach(otherContentBox => {
    ...
    async showResults(index, latitudeDMS, longitudeDMS, latitude, longitude) {
        ...
        if (resultDiv) {
            resultDiv.querySelector('.accordion-coord-changer__content__lat-result').textContent = latitudeDMS;
            resultDiv.querySelector('.accordion-coord-changer__content__lon-result').textContent = longitudeDMS;

            try {
                const location = await this.getCity(latitude, longitude);
                resultDiv.querySelector('.accordion-coord-changer__label__location').textContent = location;
            } catch (error) {
                // console.log(`Wystąpił błąd: ${error.message}`);
            }

            // Dodanie event listenera do ikony zmiany jednostki
            const changeUnitIcon = resultDiv.querySelector('.accordion-coord-changer__content__change-unit');
            if (changeUnitIcon) {
    ...
    initCopyFunctionality() {
        this.accordionContainer.addEventListener('click', function(event) {
            if (event.target.classList.contains('fa-copy')) {
                const copyButton = event.target.closest('.accordion-coord-changer__content__copy');
                if (copyButton) {
    ...
    toggleUnits(index, latitudeDMS, longitudeDMS, latitude, longitude) {
        ...
        if (resultDiv) {
            if (this.unitsDMS) {
                ...
                resultDiv.querySelector('.accordion-coord-changer__content__lat-result').textContent = latitude.toFixed(3);
                resultDiv.querySelector('.accordion-coord-changer__content__lon-result').textContent = longitude.toFixed(3);
            }
        }
    }
    ...
    clearResults(){
        document.querySelectorAll('.accordion-coord-changer__label__location').forEach(locationDiv => {
            locationDiv.textContent = '';
        });
        document.querySelectorAll('.accordion-coord-changer__content__lat-result').forEach(latResultDiv => {
            latResultDiv.textContent = '';
        });
        document.querySelectorAll('.accordion-coord-changer__content__lon-result').forEach(lonResultDiv => {
